...
// My first solution
type FirstMyAwaited<T extends Promise<unknown> |{ then: (onfulfilled: (arg: unknown) => unknown) => unknown}> = 
  T extends Promise<Promise<infer X>>
  ? MyAwaited<Promise<X>>
    : T extends Promise<infer W>
      ? W
      : (T extends { then: (onfulfilled: (arg: infer A) => unknown) => unknown} ? A : never)

// Solution after learning PromiseLike exists
